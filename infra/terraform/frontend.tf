# ── Static SPA hosting (Vite build → S3 → CloudFront) ──
...
resource "aws_cloudfront_distribution" "frontend" {
  ...
  # Route /api/* and /media/* to their real origins so the SPA can use relative URLs.
  origin {
    domain_name = replace(replace(aws_apprunner_service.api.service_url, "https://", ""), "/", "")
    origin_id   = "api"
