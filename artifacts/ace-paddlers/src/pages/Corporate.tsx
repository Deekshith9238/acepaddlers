...
function CorporateContent() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    // "25 people" / "about 30" — the field is free text by design, so pull the
    // first number out of it and keep the original wording in `extra`.
    const guests = Number.parseInt(form.groupSize.replace(/\D/g, ""), 10);
    createEnquiry.mutate(
      {
        data: {
          ...
          numGuests: Number.isFinite(guests) && guests > 0 ? guests : null,
          message: form.message.trim() || null,
          source: "corporate",
          analyticsSessionId: analyticsSessionId(),
          extra: {
            "Activity of interest": form.activity,
            "Group size (as entered)": form.groupSize,
      ...
      },
      {
        ...
        onError: () => setError(`Something went wrong sending that. Please call us on ${biz.phones[0]} and we'll sort it out.`),
      },
    );
  };
  ...
  return (
    <>
      <PageMeta
        ...
        {/* Hero */}
        <PageHero page="corporate" fallbackImage="/images/barpole-rafting-1.jpg">
                ...
                <Animate key={i} variant="up" delay={i * 100}>
                  <div className={`rounded-2xl overflow-hidden border h-full flex flex-col ${pkg.highlight ? "ring-2" : ""}`}
                    ...
                    <div className="p-8 bg-white flex-1 flex flex-col">
                      <div className="mb-6">
                        <h3 className="text-2xl mb-1" style={{ fontFamily: "var(--app-font-serif)", color: C.text }}>{pkg.name}</h3>
                        <div className="text-sm mb-1" style={{ color: "#5a8ea8" }}>{pkg.size} · {pkg.duration}</div>
                        <div className="text-2xl font-bold mt-3" style={{ color: C.deepOcean, fontFamily: "var(--app-font-serif)" }}>{pkg.price}</div>
                      </div>
            ...
            <Animate variant="up">
              ...
              {submitted ? (
                <div className="text-center py-16 rounded-2xl" style={{ backgroundColor: C.riverTeal + "12", border: `1.5px solid ${C.riverTeal}33` }}>
                  ...
                  <h3 className="text-2xl mb-2" style={{ fontFamily: "var(--app-font-serif)", color: C.text }}>Inquiry Received!</h3>
                ...
                <form onSubmit={submit} className="bg-white rounded-2xl p-8 border space-y-5" style={{ borderColor: C.mutedBorder }}>
                  ...
                  <button type="submit"
                    ...
                    className="w-full rounded-full py-4 font-semibold text-white transition-all hover:-translate-y-0.5 disabled:opacity-60 disabled:translate-y-0"
                    style={{ backgroundColor: C.riverTeal }}>
                    {createEnquiry.isPending ? "Sending…" : "Submit Group Inquiry"}
                  </button>
                </form>
              )}
            </Animate>
