...
export default function Gallery() {
  return <EditablePage slug="gallery"><GalleryContent /></EditablePage>;
}
...
function GalleryContent() {
  ...
  const lightbox: GalleryItem | null = lightboxIndex != null ? (visible[lightboxIndex] ?? null) : null;
  const closeLightbox = () => setLightboxIndex(null);
  const showPrev = () => setLightboxIndex(i => (i == null ? i : (i - 1 + visible.length) % visible.length));
  const showNext = () => setLightboxIndex(i => (i == null ? i : (i + 1) % visible.length));

  // Auto-advance while the lightbox is open; restarts on every manual navigation.
  useEffect(() => {
    if (lightboxIndex == null || visible.length < 2) return;
  ...
  return (
    <>
      <PageMeta
        ...
        />
        {/* Hero */}
        <PageHero page="gallery" fallbackImage="/images/badra-rafting-2.jpg">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="max-w-xl mx-auto text-white/85 text-lg">
        ...
        <div className="sticky top-16 z-30 px-6 py-4 flex flex-wrap justify-center gap-2"
          ...
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => { setActive(cat); closeLightbox(); }}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div className="relative max-w-4xl w-full" onClick={e => e.stopPropagation()}>
              ...
              {visible.length > 1 && (
                <>
                  <button
                    className="absolute left-2 top-1/2 -translate-y-1/2 z-10 w-11 h-11 rounded-full flex items-center justify-center text-white transition-colors"
                    style={{ backgroundColor: "rgba(6,18,28,0.55)" }}
                    onClick={showPrev} aria-label="Previous photo">
                    <ChevronLeft className="w-6 h-6" />
                  </button>
                  <button
                    className="absolute right-2 top-1/2 -translate-y-1/2 z-10 w-11 h-11 rounded-full flex items-center justify-center text-white transition-colors"
                    style={{ backgroundColor: "rgba(6,18,28,0.55)" }}
                    onClick={showNext} aria-label="Next photo">
